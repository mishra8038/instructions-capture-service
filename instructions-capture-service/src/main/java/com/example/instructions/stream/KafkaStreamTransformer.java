 ...
 * This is a high throughput processor that replaces Kafka Listener
 */
@Service
@Profile("kstreams")
public class KafkaStreamTransformer {

    @Value("${app.kafka.topics.raw:instructions.inbound}")
    ...
    @Value("${app.cache.max-entries:250000}")
    private int maxEntries;

    @Value("${app.cache.hmac-secret:dev-secret}")
    ...
    public KStream<String, CanonicalTrade> tradeTransformPipeline(StreamsBuilder builder) {
        KStream<String, CanonicalTrade> source =
                builder.stream(rawTopic, Consumed.with(Serdes.String(), tradeSerde));

        // 1) Convert nulls out; 2) in-memory hot-window dedup + cache hit short-circuit;
        // 3) transform; 4) key by privacy-safe HMAC account for partition affinity; 5) publish
        KStream<String, CanonicalTrade> transformed =
                source
                        .filter((k, v) -> v != null)
        ...
        transformed.to(outTopic, Produced.with(Serdes.String(), tradeSerde));
        return transformed;
    }
}
